...
class RamasseLogicTests(unittest.TestCase):
    ...
    def test_create_day_respecte_le_jour_de_semaine(self):
        ...
        rs.create_day(self.conn, "BA", self.lundi_amj, weekday=0)
        stores = rs.get_scheduled_stores(self.conn, "BA", self.lundi_amj)
        self.assertEqual([m for m, _ in stores], [10])
        lignes = rs.get_store_lines(self.conn, "BA", self.lundi_amj, 10)
        self.assertEqual(len(lignes), 3)

    def test_create_day_mardi_ouvre_magasin_20(self):
        rs.create_day(self.conn, "BA", self.mardi_amj, weekday=1)
        stores = rs.get_scheduled_stores(self.conn, "BA", self.mardi_amj)
        self.assertEqual([m for m, _ in stores], [10, 20])

    def test_add_store_hors_planning(self):
        rs.create_day(self.conn, "BA", self.lundi_amj, weekday=0)
        rs.add_store(self.conn, "BA", self.lundi_amj, 20)
        stores = rs.get_scheduled_stores(self.conn, "BA", self.lundi_amj)
        self.assertEqual(sorted(m for m, _ in stores), [10, 20])

    def test_navigation_suivant_precedent(self):
        rs.create_day(self.conn, "BA", self.mardi_amj, weekday=1)
        premier = rs.get_first_store(self.conn, "BA", self.mardi_amj)
        self.assertEqual(premier, 10)
        suivant = rs.get_adjacent_store(self.conn, "BA", self.mardi_amj, premier, "S")
        self.assertEqual(suivant, 20)
        plus_loin = rs.get_adjacent_store(self.conn, "BA", self.mardi_amj, suivant, "S")
        self.assertIsNone(plus_loin)  # "Pas de suivant, fin de liste"
        retour = rs.get_adjacent_store(self.conn, "BA", self.mardi_amj, suivant, "P")
    ...
    def test_cumul_totaux_toutes_lignes_meme_article(self):
        rs.create_day(self.conn, "BA", self.mardi_amj, weekday=1)
        lignes10 = rs.get_store_lines(self.conn, "BA", self.mardi_amj, 10)
        lignes20 = rs.get_store_lines(self.conn, "BA", self.mardi_amj, 20)
        ...
        rs.save_lines(self.conn, lignes10)
        rs.save_lines(self.conn, lignes20)

        totaux = rs.cumul_totals(self.conn, "BA", self.mardi_amj, ["0119000"])
    ...
    def test_totaux_hors_magasin_pour_mise_a_jour_en_direct(self):
        ...
        base = rs.totaux_hors_magasin(self.conn, "BA", self.mardi_amj, 10, ["0119000"])
    ...
    def test_empty_stores_et_day_total(self):
        rs.create_day(self.conn, "BA", self.lundi_amj, weekday=0)
        self.assertEqual(rs.day_total(self.conn, "BA", self.lundi_amj), 0.0)
        self.assertEqual(rs.empty_stores(self.conn, "BA", self.lundi_amj), ["Magasin Dix"])

        lignes = rs.get_store_lines(self.conn, "BA", self.lundi_amj, 10)
        lignes[0]["qte"] = 7
        rs.save_lines(self.conn, lignes)

        self.assertEqual(rs.empty_stores(self.conn, "BA", self.lundi_amj), [])
        self.assertEqual(rs.day_total(self.conn, "BA", self.lundi_amj), 7.0)

    def test_delete_day_si_tout_est_a_zero(self):
        rs.create_day(self.conn, "BA", self.lundi_amj, weekday=0)
        self.assertTrue(rs.day_exists(self.conn, "BA", self.lundi_amj))
        rs.delete_day(self.conn, "BA", self.lundi_amj)
        self.assertFalse(rs.day_exists(self.conn, "BA", self.lundi_amj))
    ...
    def test_export_csv_genere_les_2_fichiers(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            nom_achat, nb_achat, nom_rebut, nb_rebut = rs.export_csv(
                self.conn, "BA", self.lundi_amj, tmp, "58"
            )
    ...
    def test_suggest_default_date_lundi_puis_mardi(self):
        rs.create_day(self.conn, "BA", self.lundi_amj, weekday=0)
        date_suivante, weekday = rs.suggest_default_date(self.conn, "BA")
...
class FlaskRoutesSmokeTests(unittest.TestCase):
    ...
    def test_saisie_invalide_refusee_avec_message_et_rien_enregistre(self):
        ...
        for valeur_invalide in ("ABCD", "100..200"):
            ...
            lignes = rs.get_store_lines(self.conn, "BA", "2026-08-25", 10)
